...
export class RoleCatalogService implements CatalogAdapter {
  ...
  findAll(query: PaginationQuery): Observable<PaginatedResponse<CatalogItem>> {
    ...
    return this.http.get<PaginatedResponse<CatalogItem>>(`${BASE_URL}/getRoles`, { params });
  }

  create(name: string): Observable<CatalogItem> {
    return this.http.post<CatalogItem>(`${BASE_URL}/createRole`, { name });
  }

  update(id: number, payload: CatalogUpdatePayload): Observable<CatalogItem> {
    return this.http.patch<CatalogItem>(`${BASE_URL}/upadateRole/${id}`, payload);
  }
}
