...
const BASE_URL = `${environment.apiUrl}/payment-methods`;

@Injectable({ providedIn: 'root' })
export class PaymentMethodCatalogService implements CatalogAdapter {
  ...
  findAll(query: PaginationQuery): Observable<PaginatedResponse<CatalogItem>> {
    ...
    return this.http.get<PaginatedResponse<CatalogItem>>(`${BASE_URL}/getPaymentMethods`, { params });
  }

  create(name: string): Observable<CatalogItem> {
    return this.http.post<CatalogItem>(`${BASE_URL}/createPaymentMethod`, { name });
  }

  update(id: number, payload: CatalogUpdatePayload): Observable<CatalogItem> {
    return this.http.patch<CatalogItem>(`${BASE_URL}/updatePaymentMethod/${id}`, payload);
  }
}
